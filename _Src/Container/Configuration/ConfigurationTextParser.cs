...
{
	internal static class ConfigurationTextParser
	{
		public static Action<ContainerConfigurationBuilder> Parse(Type[] types, string configuration)
		{
			var parseItems = SplitWithTrim(configuration.Replace("\r\n", "\n"), "\n").Select(Parse).ToArray();
